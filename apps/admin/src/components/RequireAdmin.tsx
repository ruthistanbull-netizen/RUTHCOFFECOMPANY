...
function RRHubGateMark() {
  return (
    <svg
      ...
      aria-hidden="true"
      focusable="false"
      className="h-[74px] w-[80px] text-[#FBF3E6]"
      fill="currentColor"
    >
...
export function RequireAdmin({ children }: { children: ReactNode }) {
  ...
  return (
    <main className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden bg-[#111111] px-5 py-[max(2rem,env(safe-area-inset-top))] text-[#FBF3E6]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_32%,rgba(251,243,230,0.07),transparent_34%)]" />
      <div className="relative w-full max-w-[430px] text-center">
        <div className="mx-auto flex items-center justify-center">
          <RRHubGateMark />
        </div>
        <p className="mt-2 text-[11px] font-semibold tracking-[0.22em] text-[#FBF3E6]/55">RR HUB</p>

        {state === "checking" ? (
          <div className="mt-8">
            <h1 className="text-[22px] font-semibold tracking-[-0.02em] text-[#FBF3E6]">Güvenli oturum hazırlanıyor</h1>
            <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-[#FBF3E6]/58">
              Yönetici oturumun ve erişim yetkin doğrulanıyor.
            </p>
            <div className="mx-auto mt-6 h-9 w-9 animate-spin rounded-full border-2 border-[#FBF3E6]/18 border-t-[#FBF3E6]" />
            <div className="mt-4 flex items-center justify-center gap-2 text-[10px] text-[#FBF3E6]/45">
              <ShieldCheck className="h-3.5 w-3.5" /> RR HUB güvenli bağlantısı
            </div>
          </div>
        ) : (
          <div className="mt-8 rounded-[22px] border border-[#FBF3E6]/12 bg-[#242424] p-5 text-left shadow-[0_24px_70px_rgba(0,0,0,0.35)]">
            <h1 className="text-lg font-semibold text-[#FBF3E6]">
              {recoverable ? "Kimlik servisi geçici olarak yavaş" : "Yönetici erişimi gerekli"}
            </h1>
            <p className="mt-2 text-sm leading-relaxed text-[#FBF3E6]/58">
