...
function AuthGateRRMark() {
  return (
    <svg viewBox="0 0 320 297" aria-hidden="true" className="h-[68px] w-[74px] text-[#F4F0E8]" fill="currentColor">
...
export function AdminAuthGate({ children }: { children: ReactNode }) {
  ...
  if (!ready) {
    return (
      <main className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden bg-[#141414] px-5 text-[#F4F0E8]">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_34%,rgba(244,240,232,0.07),transparent_34%)]" />
        <div className="relative text-center">
          <div className="flex justify-center"><AuthGateRRMark /></div>
          <p className="mt-2 text-[10px] font-semibold tracking-[0.22em] text-[#F4F0E8]/50">RR HUB</p>
          <h1 className="mt-7 text-xl font-semibold text-[#F4F0E8]">Panel hazırlanıyor</h1>
          <p className="mt-2 text-sm text-[#F4F0E8]/55">{error || "Güvenli yönetici oturumu doğrulanıyor."}</p>
          <div className="mx-auto mt-5 h-8 w-8 animate-spin rounded-full border-2 border-[#F4F0E8]/18 border-t-[#F4F0E8]" />
        </div>
      </main>
    );
  }
