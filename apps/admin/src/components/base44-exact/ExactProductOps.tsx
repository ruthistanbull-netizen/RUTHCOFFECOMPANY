...
function ExactProductOpsContent({ mode }: { mode: Mode }) {
  ...
  const columns: ExactColumn<Product>[] = [
    ...
    { key: "material", label: "Kahve Türü", render: (product) => <span className="text-xs text-muted">{product.material || "—"}</span> },
  ...
  return <div className="space-y-4 animate-fade-in" data-exact-base44-page={`products-${mode}`}>
    ...
    <div className="space-y-3"><ExactSearchInput value={query} onChange={setQuery} placeholder="Ürün, slug, kahve türü veya kavrum profili ara..." /><ExactFilterBar chips={[{ key: "status", label: "Tüm Durumlar", value: statusFilter === "all" ? null : statusFilter, options: [{ label: "Aktif", value: "active" }, { label: "Taslak", value: "draft" }, { label: "Arşiv", value: "archived" }] }, { key: "stock", label: "Tüm Stoklar", value: stockFilter === "all" ? null : stockFilter, options: [{ label: "Stokta", value: "in" }, { label: "Düşük stok", value: "low" }, { label: "Stok yok", value: "out" }] }]} onChipChange={(key, value) => key === "status" ? setStatusFilter(value || "all") : setStockFilter(value || "all")} /></div>
