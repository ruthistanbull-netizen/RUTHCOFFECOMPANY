    ...
    >
      {selected ? <div className="space-y-5">
        ...
        <section className="space-y-3">
          ...
          {threadReplies.map((reply) => <div className={reply.direction === "outbound" ? "flex justify-end" : "flex justify-start"} key={reply.id}><div className={`ruth-type-body max-w-[88%] p-4 radius-control text-main whitespace-pre-wrap ${reply.direction === "outbound" ? "bg-accent-soft" : "bg-surface-secondary"}`}><p>{reply.body}</p><span className="ruth-type-caption mt-2 block text-subtle">{reply.direction === "outbound" ? "ROSTA Coffee Co." : selected.name || "Müşteri"} · {dateTime(reply.createdAt)}</span></div></div>)}
        </section>
