...
function ExactRuthieOrb({ state, size = 112 }: { state: RuthieState; size?: number }) {
  ...
  return <div className="relative shrink-0" style={{ width: size, height: size }} role="img" aria-label={`ROSTA Insight: ${statusLabel(state)}`}>
    ...
    <div className="absolute inset-0 rounded-full overflow-hidden" style={{ background: "radial-gradient(circle at 35% 28%, #C8A77D 0%, #C94A40 42%, #38251C 100%)", boxShadow: `inset 0 ${size * .1}px ${size * .18}px rgba(255,255,255,.25), inset 0 -${size * .08}px ${size * .14}px rgba(17,17,17,.22)`, animation }}>
      ...
      <div className="absolute rounded-full pointer-events-none" style={{ bottom: "4%", left: "26%", width: "48%", height: "14%", background: "radial-gradient(ellipse, rgba(251,243,230,.3) 0%, transparent 70%)", filter: "blur(1px)" }} />
