...
const sizePresets = [
  { value: "250 g · Çekirdek", label: "250 g · Çekirdek" },
  { value: FILTER_USAGE_VALUE, label: "250 g · Filtre öğütüm" },
  { value: "250 g · Espresso öğütüm", label: "250 g · Espresso öğütüm" },
  { value: "500 g · Çekirdek", label: "500 g · Çekirdek" },
  { value: "1 kg · Çekirdek", label: "1 kg · Çekirdek" },
];
const carePresets = [
  { value: "Serin, kuru ve doğrudan güneş görmeyen yerde; hava almayan ambalajda saklayın. Tazelik için paketi açtıktan sonra mümkün olduğunca kısa sürede tüketin.", label: "Standart kahve saklama" },
  { value: "Paketi her kullanımdan sonra sıkıca kapatın; kahveyi ısı, nem, ışık ve yoğun kokulardan uzak tutun.", label: "Açılmış paket saklama" },
  { value: "Öğütülmüş kahveyi aroma kaybını azaltmak için hava almayan ambalajda saklayın ve mümkün olduğunca taze tüketin.", label: "Öğütülmüş kahve saklama" },
];
const emptyForm: ProductForm = {
  ...
  short_description: "",
  description: "",
  size_usage: sizePresets[0].value,
  ...
  return <div className="space-y-4 animate-fade-in" data-exact-base44-page="product-studio">
    ...
    <div className="grid items-start gap-4 xl:grid-cols-[340px_1fr]">
      ...
      <div className="space-y-4">
        ...
        <ExactDataCard title="Paket, Öğütüm ve Saklama">
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3"><ExactField label="Paket / öğütüm şablonu"><select value={sizeMode} onChange={(event) => { const value = event.target.value; setSizeMode(value); if (value !== "__custom__") updateForm({ size_usage: value }); }} className={exactFormInputClass}>{sizePresets.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}<option value="__custom__">Özel metin yaz</option></select></ExactField>{sizeMode === "__custom__" ? <ExactField label="Özel paket / kullanım"><textarea value={form.size_usage} onChange={(event) => updateForm({ size_usage: event.target.value })} className={`${exactFormInputClass} min-h-28`} /></ExactField> : null}{form.size_usage === FILTER_USAGE_VALUE ? <div className="rounded-[var(--radius-control)] border border-accent/20 bg-accent-soft p-3"><p className="text-xs font-semibold text-accent">Filtre öğütüm seçildi</p><p className="mt-1 text-[10px] leading-relaxed text-muted">Storefront ürün detayında paket / öğütüm bilgisi olarak gösterilir.</p></div> : null}</div>
            <div className="space-y-3"><ExactField label="Saklama önerisi şablonu"><select value={careMode} onChange={(event) => { const value = event.target.value; setCareMode(value); if (value !== "__custom__") updateForm({ care_advice: value }); }} className={exactFormInputClass}>{carePresets.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}<option value="__custom__">Özel saklama metni yaz</option></select></ExactField>{careMode === "__custom__" ? <ExactField label="Özel saklama önerisi"><textarea value={form.care_advice} onChange={(event) => updateForm({ care_advice: event.target.value })} className={`${exactFormInputClass} min-h-28`} /></ExactField> : <div className="rounded-[var(--radius-control)] bg-surface-secondary p-3 text-xs leading-relaxed text-muted">{form.care_advice}</div>}</div>
          </div>
        </ExactDataCard>
