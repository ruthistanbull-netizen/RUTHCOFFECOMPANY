...
const SIZE_GUIDE_VALUE = "Kolye ölçü fotoğrafı";
const PRODUCT_CACHE_STALE_MS = 2 * 60 * 60_000;
const sizePresets = [
  { value: "Standart ürün ölçüsü", label: "Standart ürün ölçüsü" },
  { value: SIZE_GUIDE_VALUE, label: "Kolye / zincir ölçü tablosunu göster" },
  { value: "40 / 45 / 50 / 55 / 60 cm zincir seçenekleri", label: "Zincir: 40 / 45 / 50 / 55 / 60 cm" },
  { value: "Ayarlanabilir yüzük gövdesi", label: "Ayarlanabilir yüzük" },
  { value: "Ayarlanabilir bileklik ölçüsü", label: "Ayarlanabilir bileklik" },
];
const carePresets = [
  { value: "Parfüm, su ve kimyasal temasından kaçının. Kullanmadığınızda kutusunda saklayın.", label: "Standart takı bakımı" },
  { value: "925 ayar gümüş ürününüzü parfüm, krem, deniz ve havuz suyundan uzak tutun. Kararma olduğunda yumuşak gümüş temizleme beziyle nazikçe silin ve hava almayan kutusunda saklayın.", label: "925 gümüş bakım önerisi" },
  { value: "Kaplamanın ömrünü korumak için ürününüzü su, parfüm, krem ve kimyasallarla temas ettirmeyin. Kullanım sonrasında kuru, yumuşak bir bezle silerek kutusunda saklayın.", label: "Altın kaplama bakım önerisi" },
  { value: "Çelik ürününüzü kullanım sonrasında yumuşak bir bezle kurulayın. Kimyasal ve yoğun klor temasından kaçının; ayrı bir kesede saklayın.", label: "Çelik bakım önerisi" },
  { value: "Pirinç ürünlerde zamanla doğal patina oluşabilir. Parfüm, su ve kimyasal temasından kaçının; kuru ve yumuşak bir bezle temizleyin.", label: "Pirinç bakım önerisi" },
];
const emptyForm: ProductForm = {
  ...
  short_description: "",
  description: "",
  size_usage: "Standart ürün ölçüsü",
      ...
      <div className="space-y-4">
        ...
        <ExactDataCard title="Ölçü, Kullanım ve Bakım">
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3"><ExactField label="Ölçü ve kullanım şablonu"><select value={sizeMode} onChange={(event) => { const value = event.target.value; setSizeMode(value); if (value !== "__custom__") updateForm({ size_usage: value }); }} className={exactFormInputClass}>{sizePresets.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}<option value="__custom__">Özel metin yaz</option></select></ExactField>{sizeMode === "__custom__" ? <ExactField label="Özel ölçü ve kullanım"><textarea value={form.size_usage} onChange={(event) => updateForm({ size_usage: event.target.value })} className={`${exactFormInputClass} min-h-28`} /></ExactField> : null}{form.size_usage === SIZE_GUIDE_VALUE ? <div className="overflow-hidden rounded-[var(--radius-control)] border border-accent/20 bg-accent-soft"><div className="p-3"><p className="text-xs font-semibold text-accent">Zincir ölçü tablosu aktif</p><p className="mt-1 text-[10px] leading-relaxed text-muted">Storefront ürün detayında mevcut <code>/necklace-size-guide.jpg</code> görseli gösterilecek. Bu seçenek yalnız kolye ve zincir ürünlerinde görünür.</p></div><img src="https://ruthistanbull.tr/necklace-size-guide.jpg" alt="Zincir ölçü tablosu" className="max-h-72 w-full bg-white object-contain" onError={(event) => { event.currentTarget.style.display = "none"; }} /></div> : null}</div>
