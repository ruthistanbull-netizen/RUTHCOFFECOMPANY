...
export function ExactCRM() {
  ...
  return <div className="space-y-4 animate-fade-in" data-exact-base44-page="crm">
    ...
    <ExactDetailDrawer open={Boolean(selected)} onClose={requestClose} title={selected ? `CRM · #${selected.order_no}` : "CRM"} subtitle={selected?.customer_name} width={660} footer={selected ? <div className="flex gap-2"><ExactButton variant="secondary" size="sm" className="flex-1" onClick={requestClose} disabled={saving}>Kapat</ExactButton><ExactButton size="sm" className="flex-1" onClick={() => void saveLifecycle()} loading={saving}><Save className="h-4 w-4" /> Kaydet</ExactButton></div> : null}>
      {selected ? <div className="space-y-5"><section><div className="flex items-center gap-3"><ExactAvatar name={selected.customer_name || "?"} size="lg" /><div><p className="ruth-type-card-title text-main">{selected.customer_name}</p>{selected.customer_phone ? <a href={`tel:${selected.customer_phone}`} className="ruth-type-caption flex items-center gap-1 text-muted"><Phone className="h-3 w-3" /> {selected.customer_phone}</a> : null}{selected.customer_email ? <a href={`mailto:${selected.customer_email}`} className="ruth-type-caption flex items-center gap-1 text-muted"><Mail className="h-3 w-3" /> {selected.customer_email}</a> : null}</div></div><div className="mt-3 p-3 radius-small bg-surface-secondary"><p className="ruth-type-label flex items-center gap-1 uppercase text-subtle"><MapPin className="h-3 w-3" /> Teslimat adresi</p><p className="ruth-type-body mt-1 text-main">{address(selected)}</p></div></section><div className="grid grid-cols-3 gap-2"><div className="p-3 radius-small bg-surface-secondary"><p className="ruth-type-label text-subtle">Sipariş</p><p className="ruth-type-price font-bold text-main">{money(selected.total_amount, selected.currency)}</p></div><div className="p-3 radius-small bg-surface-secondary"><p className="ruth-type-label text-subtle">Durum</p><ExactStatusBadge status={selected.status} label={statusLabel(selected.status)} size="sm" /></div><div className="p-3 radius-small bg-surface-secondary"><p className="ruth-type-label text-subtle">Ürün</p><p className="ruth-type-table font-bold text-main">{(selected.order_items || []).reduce((sum, item) => sum + Number(item.quantity || 0), 0)} adet</p></div></div><ExactField label="Operasyon / müşteri notu"><textarea rows={5} value={draft.note} onChange={(event) => setDraft((current) => ({ ...current, note: event.target.value }))} className={`${exactFormInputClass} min-h-28`} placeholder="Müşteri tercihi, kahve profili, görüşme sonucu..." /></ExactField><ExactField label="Hatırlatma notu"><textarea rows={3} value={draft.reminderNote} onChange={(event) => setDraft((current) => ({ ...current, reminderNote: event.target.value }))} className={`${exactFormInputClass} min-h-20`} placeholder="Örn. Tadım geri bildirimi için müşteriyi ara" /></ExactField><ExactField label="Hatırlatma zamanı"><input type="datetime-local" value={draft.reminderAt} onChange={(event) => setDraft((current) => ({ ...current, reminderAt: event.target.value }))} className={exactFormInputClass} /></ExactField><div className="flex flex-wrap gap-2"><ExactButton variant="tertiary" size="sm" onClick={() => setDraft((current) => ({ ...current, reminderAt: preset(0, 15) }))}>Bugün 15:00</ExactButton><ExactButton variant="tertiary" size="sm" onClick={() => setDraft((current) => ({ ...current, reminderAt: preset(1, 10) }))}>Yarın 10:00</ExactButton><ExactButton variant="tertiary" size="sm" onClick={() => setDraft((current) => ({ ...current, reminderAt: preset(3, 10) }))}>3 gün sonra</ExactButton><ExactButton variant="tertiary" size="sm" onClick={() => setDraft((current) => ({ ...current, reminderAt: "", reminderNote: "" }))}>Temizle</ExactButton></div></div> : null}
    </ExactDetailDrawer>
  </div>;
}
