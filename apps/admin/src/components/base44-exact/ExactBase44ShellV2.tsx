...
function Brand({ collapsed = false }: { collapsed?: boolean }) {
  return (
    <Link href="/" className={exactCx("flex min-w-0 items-center gap-2.5", collapsed && "justify-center")}>
      ...
      {!collapsed ? <div className="min-w-0"><p className="ruth-type-card-title truncate text-main">Ruth Commerce</p><p className="ruth-type-caption truncate text-subtle">Control Room</p></div> : null}
    </Link>
  );
}
...
function MobileDrawerBrand({ onClick }: { onClick: () => void }) {
  return (
    <Link
      href="/"
      aria-label="Ruth Commerce ana sayfası"
      onClick={onClick}
