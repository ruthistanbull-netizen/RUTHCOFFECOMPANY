...
const emptyData: DashboardState = {
  summary: { orders: 0, paidOrders: 0, revenue: 0, sessions: 0, carts: 0, checkoutReached: 0, returns: 0, products: 0, conversionRate: 0, customers: 0, newCustomers: 0 },
  recentOrders: [],
...
function SalesChart({ data }: { data: Array<{ day: string; sales: number }> }) {
  ...
  const points = data.map((point, index) => ({
    ...point,
    x: padding.left + (index * (width - padding.left - padding.right)) / Math.max(1, data.length - 1),
    y: padding.top + (1 - point.sales / max) * (height - padding.top - padding.bottom),
  }));
  ...
  return (
    <div className="w-full overflow-hidden">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-[220px]" role="img" aria-label="Son yedi gün satış grafiği">
        <defs><linearGradient id="exactBase44SalesGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="hsl(var(--accent))" stopOpacity="0.3" /><stop offset="100%" stopColor="hsl(var(--accent))" stopOpacity="0" /></linearGradient></defs>
        ...
        <polygon points={area} fill="url(#exactBase44SalesGradient)" />
...
function RangeSelect({ value, onChange }: { value: AdminDateRangeValue; onChange: (next: AdminDateRangeValue) => void }) {
  return (
    <select aria-label="Tarih aralığı" value={value.range} onChange={(event) => onChange({ ...value, range: event.target.value as AdminDateRangeKey })} className="h-8 px-3 radius-small bg-surface-secondary border border-border-subtle text-xs text-main focus:outline-none focus:ring-2 focus:ring-accent">
...
export function ExactOverview() {
  ...
  return (
    <div className="space-y-4 animate-fade-in" data-exact-base44-page="overview">
      ...
      {loading ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3"><ExactSkeleton className="h-[146px]" /><ExactSkeleton className="h-[146px] lg:col-span-2" /></div>
      ) : (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <ExactMetricCard label="Toplam Sipariş" value={Number(summary.orders || 0)} icon={ShoppingBag} />
        <ExactMetricCard label="İşlem Gerektiren" value={attention} icon={AlertCircle} />
        <ExactMetricCard label="Toplam Ürün" value={Number(summary.products || 0)} icon={Package} />
        <ExactMetricCard label="Açık İade" value={data.openReturns} icon={RotateCcw} />
        <ExactMetricCard label="Dönüşüm Oranı" value={Number(summary.conversionRate || 0)} format="percent" icon={Percent} />
        <ExactMetricCard label="Ortalama Sepet" value={averageOrder} format="currency" icon={CircleDollarSign} />
        <ExactMetricCard label="Başarılı Ödeme" value={Number(data.paymentSummary.successfulCount || summary.paidOrders || 0)} icon={CreditCard} />
        <ExactMetricCard label="Oturum" value={Number(summary.sessions || 0)} icon={UserPlus} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <ExactDataCard title="Satış — Son 7 Gün" className="lg:col-span-2" bodyClassName="pt-2"><SalesChart data={sales} /></ExactDataCard>
        <ExactDataCard title="Sipariş Durumu"><div className="space-y-2.5">{orderDistribution.map((item) => { const percentage = (item.count / distributionTotal) * 100; return <div key={item.label}><div className="flex items-center justify-between mb-1"><ExactStatusBadge status={item.label} label={item.label} tone={item.tone} dot={false} size="sm" /><span className="text-xs font-semibold text-main">{item.count.toLocaleString("tr-TR")}</span></div><div className="h-1.5 rounded-full bg-surface-tertiary overflow-hidden"><div className="h-full rounded-full bg-accent transition-[width] duration-500" style={{ width: `${Math.max(2, percentage)}%` }} /></div></div>; })}</div></ExactDataCard>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <ExactDataCard title="Canlı Aktivite" className="lg:col-span-2" noPadding>
          <div className="divide-y divide-border-subtle">
            {data.recentOrders.slice(0, 7).map((order) => { const presentation = statusPresentation(order); const Icon = presentation.tone === "danger" ? AlertCircle : presentation.tone === "warning" ? Hammer : presentation.tone === "accent" ? Truck : ShoppingBag; return <Link href={`/orders?q=${encodeURIComponent(order.order_no)}`} key={order.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-surface-secondary transition-colors"><div className={exactCx("flex items-center justify-center h-8 w-8 radius-small shrink-0", presentation.tone === "danger" ? "bg-danger-soft text-danger-foreground" : presentation.tone === "warning" ? "bg-warning-soft text-warning-foreground" : presentation.tone === "accent" ? "bg-accent-soft text-accent" : "bg-info-soft text-info-foreground")}><Icon className="h-4 w-4" /></div><div className="flex-1 min-w-0"><p className="text-xs font-medium text-main truncate">#{order.order_no} · {order.customer_name || "İsimsiz müşteri"}</p><p className="text-[11px] text-muted truncate">{money(order.total_amount)} · {presentation.label}</p></div><span className="text-[10px] text-subtle shrink-0">{shortDate(order.created_at)}</span></Link>; })}
        ...
        <ExactDataCard title="Acil Operasyonlar" bodyClassName="space-y-2">
          ...
          <Link href="/orders?queue=ready" className="block p-3 radius-small bg-accent-soft border border-accent/20 hover:brightness-[0.99] transition-all"><div className="flex items-center justify-between mb-1"><span className="text-xs font-semibold text-accent">Paketlemeye Hazır</span><span className="text-lg font-bold text-accent">{Number(operations.ready || 0)}</span></div><p className="text-[11px] text-muted">Gönderime hazırlanan siparişler</p></Link>
          <Link href="/orders" className="mt-1 block"><ExactButton variant="secondary" size="sm" className="w-full">Tüm kuyrukları gör <ArrowRight className="h-3.5 w-3.5" /></ExactButton></Link>
        </ExactDataCard>
      </div>
    </div>
  );
