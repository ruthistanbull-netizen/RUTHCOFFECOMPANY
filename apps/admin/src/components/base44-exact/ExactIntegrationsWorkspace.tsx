...
const integrations: Definition[] = [
  { key: "supabase", label: "Supabase", description: "Panel veritabanı, kimlik, storage ve canlı veri", scopes: ["panel", "ruthie"], icon: Database, mode: "server", setup: ["Supabase proje ayarlarında API bölümünü aç.", "Project URL ve sunucu erişimini Render/Vercel güvenli ayarlarına ekle.", "Admin servisini yeniden yayınla ve Canlı Kontrol'e bas."] },
  ...
  { key: "openai", label: "OpenAI · Ruthie", description: "Ruthie sohbeti, analiz ve onaylı panel işlemleri", scopes: ["ruthie"], icon: Sparkles, mode: "server", href: "/ruthie", setup: ["OpenAI projesi için sunucu API erişimi oluştur.", "Erişimi yalnız Render admin servisinde sakla.", "Ruthie ekranından sohbet ve onay akışını test et."] },
  ...
  { key: "google-analytics", label: "Google Analytics 4", description: "Oturum, dönüşüm ve davranış analizi", scopes: ["ruthie", "marketing"], icon: BarChart3, mode: "server", setup: ["GA4 mülkünden ölçüm kimliğini al.", "Kimliği storefront ayarlarına ekle.", "Analytics ve Ruthie raporlarında veri akışını doğrula."] },
  ...
  { key: "search-console", label: "Search Console", description: "SEO performansı ve arama görünürlüğü", scopes: ["ruthie", "marketing"], icon: BarChart3, mode: "server", setup: ["Web site mülkünü doğrula.", "Google raporlama yetkilendirmesini hazırla.", "Sunucu bağlantısını ekleyip Ruthie SEO analizini yenile."] },
  ...
  { key: "github", label: "GitHub", description: "Kod, commit, PR ve Ruthie geliştirme araçları", scopes: ["panel", "ruthie"], icon: Github, mode: "server", setup: ["Yalnız ROSTA Coffee Co. reposuna yetkili erişim oluştur.", "Repo ve erişim bilgisini admin servisine ekle.", "Ruthie geliştirme işlemlerinde bağlantıyı test et."] },
...
export function ExactIntegrationsWorkspace() {
  ...
  const load = useCallback(async (live = false) => {
    setLoading(true);
    try {
      const status = await adminRequest<{ integrations?: Partial<Record<ProviderKey, ProviderState>> }>(`/api/ruthie/integrations/status-v2${live ? "?live=meta" : ""}`, { force: true });
  ...
  return <div className="space-y-4 animate-fade-in" data-exact-base44-page="integrations">
    <ExactPageHeader title="Entegrasyonlar" subtitle="Panel ve Ruthie'nin ödeme, veri, reklam, kargo, e-posta ve deploy bağlantıları" actions={<ExactButton variant="secondary" size="sm" onClick={() => void load(true)} loading={loading}><RefreshCw className="h-4 w-4" /> Canlı Kontrol</ExactButton>} />
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><ExactMetricCard label="Bağlı" value={metrics.connected} icon={CheckCircle2} /><ExactMetricCard label="Bağlantı Bekleyen" value={metrics.waiting} icon={Unplug} /><ExactMetricCard label="Ruthie Bağlantıları" value={metrics.ruthie} icon={Sparkles} /><ExactMetricCard label="Toplam Sağlayıcı" value={integrations.length} icon={Webhook} /></div>
