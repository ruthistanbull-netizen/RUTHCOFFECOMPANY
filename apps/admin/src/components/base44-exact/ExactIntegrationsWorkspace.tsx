...
const integrations: Definition[] = [
  { key: "supabase", label: "Supabase", description: "Panel veritabanı, kimlik, storage ve canlı veri", scopes: ["panel", "insight"], icon: Database, mode: "server", setup: ["Supabase proje ayarlarında API bölümünü aç.", "Project URL ve sunucu erişimini Zeabur güvenli ayarlarına ekle.", "Admin servisini yeniden yayınla ve Canlı Kontrol'e bas."] },
  { key: "paytr", label: "PayTR", description: "Ödeme, callback, taksit ve iade işlemleri", scopes: ["panel", "insight"], icon: CreditCard, mode: "server", href: "/payments", setup: ["PayTR mağaza panelinden entegrasyon bilgilerini al.", "Bilgileri storefront ve admin servisinin güvenli ayarlarına ekle.", "Bildirim adresini production callback adresine yönlendir."] },
  { key: "basit-kargo", label: "Basit Kargo", description: "Kargo fiyatı, barkod, etiket ve ters kargo", scopes: ["panel", "insight"], icon: Truck, mode: "server", href: "/shipping", setup: ["Basit Kargo hesabından API erişimini etkinleştir.", "Erişim bilgisini admin servisinin güvenli ayarlarına ekle.", "Kargo ekranından barkod ve etiket üretimini test et."] },
  { key: "gmail", label: "Gmail", description: "Hizmet, pazarlama ve müşteri iletişimi e-postaları", scopes: ["panel", "insight", "marketing"], icon: Mail, mode: "gmail", href: "/email", setup: ["Google Cloud'da Gmail API'yi etkinleştir.", "Panel OAuth callback adresini istemciye ekle.", "Bağla düğmesine basıp Google hesabında izin ver."] },
  { key: "openai", label: "OpenAI · ROSTA Insight", description: "ROSTA Insight sohbeti, analiz ve onaylı panel işlemleri", scopes: ["insight"], icon: Sparkles, mode: "server", href: "/rosta-insight", setup: ["OpenAI projesi için sunucu API erişimi oluştur.", "Erişimi yalnız Zeabur admin servisinde sakla.", "ROSTA Insight ekranından sohbet ve onay akışını test et."] },
  { key: "meta", label: "Meta Marketing", description: "Reklam hesabı raporlama ve kampanya analizi", scopes: ["insight", "marketing"], icon: Megaphone, mode: "server", setup: ["Business Manager'da sistem kullanıcısı oluştur.", "Reklam hesabına gerekli raporlama izinlerini ver.", "Uzun ömürlü erişimi admin servisinin güvenli ayarlarına ekle."] },
  { key: "tiktok", label: "TikTok Ads", description: "TikTok reklam raporları ve performans analizi", scopes: ["insight", "marketing"], icon: Megaphone, mode: "server", setup: ["TikTok for Business geliştirici uygulaması oluştur.", "Reklam hesabı erişimini uygulamaya bağla.", "Sunucu erişimini Zeabur'a ekleyip servisi yeniden yayınla."] },
  { key: "google-analytics", label: "Google Analytics 4", description: "Oturum, dönüşüm ve davranış analizi", scopes: ["insight", "marketing"], icon: BarChart3, mode: "server", setup: ["GA4 mülkünden ölçüm kimliğini al.", "Kimliği storefront ayarlarına ekle.", "Analytics ve ROSTA Insight raporlarında veri akışını doğrula."] },
  { key: "google-tag-manager", label: "Google Tag Manager", description: "Piksel, dönüşüm ve event etiketleri", scopes: ["panel", "marketing"], icon: Tag, mode: "server", setup: ["Web container oluştur.", "Container kimliğini storefront ayarlarına ekle.", "Önizleme aracıyla sepete ekleme ve satın alma olaylarını test et."] },
  { key: "search-console", label: "Search Console", description: "SEO performansı ve arama görünürlüğü", scopes: ["insight", "marketing"], icon: BarChart3, mode: "server", setup: ["Web site mülkünü doğrula.", "Google raporlama yetkilendirmesini hazırla.", "Sunucu bağlantısını ekleyip ROSTA Insight SEO analizini yenile."] },
  ...
  { key: "github", label: "GitHub", description: "Kod, commit, PR ve ROSTA Insight geliştirme araçları", scopes: ["panel", "insight"], icon: Github, mode: "server", setup: ["Yalnız ROSTA Coffee Company reposuna yetkili erişim oluştur.", "Repo ve erişim bilgisini admin servisine ekle.", "ROSTA Insight geliştirme işlemlerinde bağlantıyı test et."] },
  { key: "zeabur", label: "Zeabur", description: "Admin ve storefront deploy, domain ve servis durumu", scopes: ["panel", "insight"], icon: Cloud, mode: "server", setup: ["ROSTA servislerinin Zeabur proje bağlantısını doğrula.", "Gizli anahtarları yalnız Zeabur servis ayarlarında sakla.", "ZEABUR_SERVICE_ID, ZEABUR_PROJECT_ID ve ZEABUR_WEB_URL değişkenlerinin mevcut olduğunu doğrula."] },
];
...
export function ExactIntegrationsWorkspace() {
  ...
  const metrics = useMemo(() => ({ connected: integrations.filter((item) => states[item.key]?.connected).length, waiting: integrations.filter((item) => !states[item.key]?.connected).length, insight: integrations.filter((item) => item.scopes.includes("insight") && states[item.key]?.connected).length }), [states]);
  ...
  return <div className="space-y-4 animate-fade-in" data-exact-base44-page="integrations">
    <ExactPageHeader title="Entegrasyonlar" subtitle="Panel ve ROSTA Insight'nin ödeme, veri, reklam, kargo, e-posta ve deploy bağlantıları" actions={<ExactButton variant="secondary" size="sm" onClick={() => void load(true)} loading={loading}><RefreshCw className="h-4 w-4" /> Canlı Kontrol</ExactButton>} />
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><ExactMetricCard label="Bağlı" value={metrics.connected} icon={CheckCircle2} /><ExactMetricCard label="Bağlantı Bekleyen" value={metrics.waiting} icon={Unplug} /><ExactMetricCard label="ROSTA Insight Bağlantıları" value={metrics.insight} icon={Sparkles} /><ExactMetricCard label="Toplam Sağlayıcı" value={integrations.length} icon={Webhook} /></div>
