...
export function CatalogManager(){
  ...
  return <>
    ...
    <AnimatePresence>{draft?<motion.div className="admin-editor-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:.18}} onMouseDown={()=>setDraft(null)}><motion.section className="admin-editor-panel" initial={{y:70,opacity:.98}} animate={{y:0,opacity:1}} exit={{y:70,opacity:0}} transition={{duration:.28,ease:[.32,.72,0,1]}} onMouseDown={e=>e.stopPropagation()}><header><div><p className="admin-kicker">KATALOG</p><h2>{draft.id?"Düzenle":"Yeni kayıt"}</h2></div><button className="admin-icon-button" onClick={()=>setDraft(null)}><X size={18}/></button></header><div className="admin-form-grid">
