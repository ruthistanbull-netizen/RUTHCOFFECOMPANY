...
export function CatalogManager(){
  ...
  return <>
    ...
    {draft?<div className="admin-editor-backdrop" onMouseDown={()=>setDraft(null)}><section className="admin-editor-panel" onMouseDown={e=>e.stopPropagation()}><header><div><p className="admin-kicker">KATALOG</p><h2>{draft.id?"Düzenle":"Yeni kayıt"}</h2></div><button className="admin-icon-button" onClick={()=>setDraft(null)}><X size={18}/></button></header><div className="admin-form-grid">
    ...
    </div><footer><button className="admin-secondary-button" onClick={()=>setDraft(null)}>Vazgeç</button><button className="admin-primary-button" disabled={busy||!draft.name.trim()} onClick={save}><Save size={15}/>{busy?"Kaydediliyor…":"Kaydet"}</button></footer></section></div>:null}
  </>;
}
