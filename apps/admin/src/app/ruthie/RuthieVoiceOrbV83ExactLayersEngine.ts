...
function createBaseLayer(source: CanvasImageSource) {
  const { canvas, context } = createCanvas(false);
...
function createExactMaskedLayer(
  source: CanvasImageSource,
  mask: CanvasImageSource,
  kind: "outer" | "middle" | "inner",
) {
  const { canvas, context } = createCanvas(true);
  const { canvas: maskCanvas, context: maskContext } = createCanvas(true);
  ...
  for (let index = 0; index < pixels.length; index += 4) {
    ...
    // HTML'deki ayrıştırılmış PNG katmanlarının alfa kanalı bu maskelerden gelir.
...
export function mountRuthieV83ExactLayers(canvas: HTMLCanvasElement, options: V83MountOptions) {
  ...
  canvas.dataset.renderer = "canvas-ruthie-v8-3-exact-four-layers";
  canvas.dataset.ruthieOrbVersion = "8.3-html-four-layers-hd-no-slices";
  ...
  // Tek yapay dış katman yerine HTML'de bulunan dört gerçek katman birlikte hazırlanır.
  void Promise.all([
    loadImage(ORB_ASSET),
    ...
    environment.images.outer = createExactMaskedLayer(base, outerMask, "outer");
    environment.images.middle = createExactMaskedLayer(base, middleMask, "middle");
    environment.images.inner = createExactMaskedLayer(base, innerMask, "inner");
    loaded = true;
    delete canvas.dataset.renderError;
    delete canvas.dataset.renderWarning;
  }).catch((error) => {
    console.error("Ruthie v8.3 exact four-layer load error", error);
    canvas.dataset.renderError = "exact-four-layer-load";
  });
