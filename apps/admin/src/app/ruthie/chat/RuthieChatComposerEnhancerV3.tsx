...
import styles from "./RuthieChatComposerEnhancerV3.module.css";
...
type CreativePurpose = "instagram_post" | "instagram_story" | "meta_ads" | "";
...
const PURPOSES = [
  {
    ...
    detail: "Feed odaklı, temiz ve dengeli",
    icon: Instagram,
  ...
  },
  {
    ...
    label: "Instagram Hikaye",
    detail: "Dikey, üst-alt güvenli alanlı",
    icon: Smartphone,
    size: "1024x1536" as ImageSize,
    platform: "Instagram story",
  },
  {
    id: "meta_ads" as const,
    label: "Meta Reklamı",
    detail: "Satış odaklı kreatif + kısa metin",
    icon: Megaphone,
    size: "1024x1536" as ImageSize,
...
const STYLE_GROUPS: StyleGroup[] = [
  { id: "brew", label: "Kahve Hazırlama Sahnesi", description: "Kahvenin hazırlanışını gerçek ve iştah açıcı biçimde göster", options: ["Espresso akışı", "V60 pour-over", "Filtre kahve demleme", "Süt köpürtme", "Latte art", "Çekirdek öğütme", "Barista hazırlığı"] },
  { id: "studio", label: "Stüdyo Ürün Fotoğrafı", description: "Temiz ve e-ticaret odaklı kahve paketi çekimi", options: ["Beyaz fon", "Krem fon", "Koyu premium fon", "Soft gölgeli", "E-ticaret uyumlu", "İzole paket görünümü"] },
  ...
  { id: "lifestyle", label: "Lifestyle / Günlük Kahve", description: "Doğal tüketim ve günlük yaşam odaklı", options: ["Kafe masası", "Evde sabah kahvesi", "Laptop yanında kahve", "Elde fincan", "Pencere önü", "Günlük şehir ortamı"] },
  { id: "flatlay", label: "Flat Lay / Masa Üstü", description: "Yukarıdan düzenlenmiş kahve kompozisyonu", options: ["Yukarıdan çekim", "Paket + çekirdek", "Fincan + paket", "Sade", "Çoklu ürün yerleşimi", "Demleme ekipmanlı"] },
  { id: "macro", label: "Makro / Detay Çekim", description: "Kahvenin doku ve tazelik detaylarını öne çıkar", options: ["Crema detayı", "Çekirdek dokusu", "Öğütülmüş kahve", "Kavrum detayı", "Buhar yakın plan", "Latte art yakın plan"] },
  { id: "hand", label: "Elde Tutulan Ürün", description: "Paketi veya fincanı elde doğal biçimde göster", options: ["Paket elde", "Fincan elde", "Çekirdek dökme", "Paketi açma", "İki elle sunum"] },
  { id: "barista", label: "Barista / Kafe Sahnesi", description: "Profesyonel kahve servis atmosferi", options: ["Espresso makinesi", "Barista portresi", "Portafiltre yakın plan", "Servis anı", "Latte art hazırlığı"] },
  { id: "stilllife", label: "Still Life / Obje Kompozisyonu", description: "Kahveyi seçili objelerle premium kurgula", options: ["Seramik fincanla", "Ahşap yüzeyde", "Metal ekipmanla", "Keten kumaşla", "Çekirdeklerle", "Demleme ekipmanıyla"] },
  { id: "editorial", label: "Editorial / Marka Çekimi", description: "Dergi ve premium kahve kampanyası estetiği", options: ["Editorial kahve çekimi", "Dramatik kompozisyon", "Dergi tarzı", "Artistik ışık", "Minimal premium"] },
  { id: "series", label: "Seri / Koleksiyon Çekimi", description: "Birden fazla kahveyi tutarlı görsel dilde sun", options: ["Sade fon", "Kavrum serisi", "Menşei serisi", "Temiz kadraj", "Premium seri sunumu"] },
  { id: "poster", label: "Reklam Afişi Tarzı", description: "Performans reklamı ve hero kreatif", options: ["Yazısız afiş", "Yazılı afiş", "Hero paket", "Hero fincan", "Dikkat çekici kompozisyon"] },
  { id: "ugc", label: "UGC / Sosyal Medya Tarzı", description: "Doğal kullanıcı çekimi hissi", options: ["Telefon kamerası hissi", "Kafe masası çekimi", "Ev yapımı kahve", "Paket açılımı", "Kullanıcı deneyimi havası"] },
  { id: "packaging", label: "Paketleme / Unboxing", description: "ROSTA paket ve kutu deneyimini göster", options: ["Kahve paketi tek başına", "Kutudan çıkarken", "Kargo kutusu içinde", "Premium ambalaj", "Paket + sticker"] },
  { id: "set", label: "Ürün Seti Görseli", description: "Birden fazla kahve veya ürünü birlikte sun", options: ["250 g + 500 g paket", "Çoklu kavrum", "Paket + fincan", "Kahve + ekipman", "Çoklu ürün premium sunum"] },
  { id: "season", label: "Sezon Temalı", description: "Döneme göre kahve atmosferi oluştur", options: ["Sabah", "Yağmurlu gün", "Yaz / iced coffee", "Sonbahar", "Gece kafe", "Yılbaşı", "Özel gün"] },
];
...
const EXTRA_GROUPS: Array<{ id: ExtraKey; label: string; options: string[] }> = [
  ...
  { id: "platform", label: "Platform", options: ["Website", "Instagram post", "Story", "Meta reklamları"] },
];
...
export function RuthieChatComposerEnhancerV3() {
  ...
  const [purpose, setPurpose] = useState<CreativePurpose>("");
  ...
  const purposeMeta = useMemo(() => PURPOSES.find((item) => item.id === purpose) || null, [purpose]);
  const PurposeSummaryIcon = purposeMeta?.icon || null;
  ...
  const menuItems = useMemo(() => {
    const items = [
      ...
      { id: "generate", label: "Fotoğraf üret", detail: "Amaç, ürün, tarz, kalite ve boyutu seç; fiyatı gör", icon: WandSparkles },
    ];
  ...
  const selectPurpose = (next: Exclude<CreativePurpose, "">) => {
    const preset = PURPOSES.find((item) => item.id === next);
    if (!preset) return;
    setPurpose((current) => {
      if (current === next) {
        setExtras((extrasCurrent) => ({
          ...extrasCurrent,
          platform: extrasCurrent.platform === preset.platform ? "" : extrasCurrent.platform,
        }));
        return "";
      }
      setSize(preset.size);
      setExtras((extrasCurrent) => ({ ...extrasCurrent, platform: preset.platform }));
      return next;
    });
    invalidateQuote();
  };
  ...
  }), [extras, purposeMeta, selectedProducts, selectedStyle]);
  ...
  const requestQuote = useCallback(async () => {
    ...
    const prompt = rawPrompt || (selectedProducts.length || selectedStyle || purpose ? "Seçili ürün, kullanım amacı ve fotoğraf ayarlarına göre premium bir görsel oluştur." : "");
    if (!prompt) {
      setImageError("Nasıl bir görsel istediğini yaz veya kullanım amacı/ürün/fotoğraf tarzı seç.");
      return;
    }
  ...
  }, [generationContext, imageStage, purpose, quality, selectedProducts.length, selectedStyle, size, targets]);
  ...
  const confirmGeneration = async () => {
    ...
    const rawPrompt = imagePromptFromTextarea(targets.textarea);
    const prompt = rawPrompt || "Seçili ürün, kullanım amacı ve fotoğraf ayarlarına göre premium bir görsel oluştur.";
  ...
  const menuOffset = imageSelected ? dockOffset + 430 : dockOffset;
          ...
          <AnimatePresence initial={false}>
            {imageSelected ? (
              ...
              >
                    ...
                    {PURPOSES.map((item) => {
                      ...
                      return (
                        <motion.button
                          ...
                          whileTap={{ scale: 0.98 }}
                        >
                          <span className={styles.purposeIcon}><Icon /></span>
                          <span className={styles.purposeCopy}><strong>{item.label}</strong><small>{item.detail}</small></span>
                          {active ? <span className={styles.purposeCheck}><Check /></span> : null}
                        </motion.button>
                      );
                    })}
                  </div>
                  {purposeMeta ? <motion.p className={styles.purposeHint} initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }}>{purposeMeta.label} seçildi · önerilen boyut otomatik ayarlandı, istersen Boyut bölümünden değiştirebilirsin.</motion.p> : null}
                </div>
                ...
                {(selectedProducts.length || selectedStyle || purposeMeta) && !imagePanel ? <motion.div className={styles.selectionSummary} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}>
                  {purposeMeta && PurposeSummaryIcon ? <span><PurposeSummaryIcon /> {purposeMeta.label}</span> : null}
                ...
                <AnimatePresence mode="wait" initial={false}>
                  {imageStage === "quote" && quote ? (
                    <motion.div key="quote" className={styles.quoteBar} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}>
                      <div><span>Tahmini görsel ücreti</span><strong>{formatUsd(quote.estimatedUsd)}</strong><small>{QUALITY_LABELS[quote.quality]} · {SIZE_LABELS[quote.size]}{purposeMeta ? ` · ${purposeMeta.label}` : ""}{quote.referenceCount ? ` · ${quote.referenceCount} ürün referansı` : ""}</small></div>
                    ...
                    <motion.div key="generating" className={styles.generatingBar} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><LoaderCircle className={styles.spin} /><span>ROSTA Insight fotoğrafı {purposeMeta ? purposeMeta.label.toLocaleLowerCase("tr-TR") : "seçili amaç"} için hazırlıyor. Ürün referansları korunuyor.</span><motion.i initial={{ x: "-100%" }} animate={{ x: "250%" }} transition={{ repeat: Infinity, duration: 1.2, ease: "easeInOut" }} /></motion.div>
                  ) : (
