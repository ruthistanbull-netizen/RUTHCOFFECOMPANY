...
function drawIntegratedRibbons(env: V83Environment, cx: number, cy: number, radius: number, time: number, mode: V83Mode, level: number) {
  const context = env.orbCtx;
  context.save();
  context.beginPath();
  context.arc(cx, cy, radius * .97, 0, Math.PI * 2);
  context.clip();
...
function applySoftOrbMask(context: CanvasRenderingContext2D, center: number, radius: number) {
  context.save();
  context.globalCompositeOperation = "destination-in";
  const maskRadius = radius * 1.31;
  const mask = context.createRadialGradient(center, center, radius * .12, center, center, maskRadius);
  mask.addColorStop(0, "rgba(255,255,255,1)");
  mask.addColorStop(.76, "rgba(255,255,255,1)");
  mask.addColorStop(.88, "rgba(255,255,255,.98)");
  mask.addColorStop(.95, "rgba(255,255,255,.72)");
  mask.addColorStop(1, "rgba(255,255,255,0)");
  context.fillStyle = mask;
  context.fillRect(0, 0, ORB_BUFFER_SIZE, ORB_BUFFER_SIZE);
  context.restore();
}

export function renderWholeOrb(env: V83Environment, time: number, mode: V83Mode, level: number, pulse: number) {
  const context = env.orbCtx;
  ...
  // The approved HTML's soft solar edge comes from the luminous outer texture,
  // not from a geometric circle. Draw it before and after the inner layers.
  drawImageLayerTo(context, env.images.outer, center, center, size * 1.035, -time * .020, pulse * 1.01, .34 + level * .08);
  drawImageLayerTo(context, env.images.base, center, center, size, time * .004, pulse, .58);
  drawImageLayerTo(
    context,
    env.images.outer,
    center,
  ...
  drawImageLayerTo(context, env.images.outer, center, center, size * 1.018, -time * .018, pulse, .24);
  drawImageLayerTo(context, env.images.middle, center, center, size * .99, time * .085, pulse, .10);
  ...
  // A feathered alpha edge replaces the old hard circular clip.
  applySoftOrbMask(context, center, radius);
}

export function drawWholeOrbWarped(
  env: V83Environment,
  cx: number,
  cy: number,
  radius: number,
  ...
  // Whole-object breathing keeps every pixel sharp. The previous 2px horizontal
  // slicing was the source of the visible scanlines on iPhone.
  ...
  // A second, low-alpha copy of the real outer texture extends through the edge,
  // creating the same sun-corona silhouette seen in the approved HTML/video.
  if (env.images.outer) {
    context.save();
    context.translate(cx, cy);
    context.rotate(-time * .016);
    context.globalCompositeOperation = "screen";
    context.globalAlpha = .17 + level * .05;
    context.imageSmoothingEnabled = true;
    context.imageSmoothingQuality = "high";
    const coronaSize = destinationSize * 1.075;
    context.drawImage(env.images.outer, -coronaSize / 2, -coronaSize / 2, coronaSize, coronaSize);
    context.restore();
  }
}
