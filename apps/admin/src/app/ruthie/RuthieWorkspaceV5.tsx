...
export function RuthieWorkspaceV5() {
  ...
  const conversationContext = useCallback(() => {
    ...
    return (conversation?.messages || []).slice(-14).map((item) => `${item.role === "user" ? "Kullanıcı" : "ROSTA Insight"}: ${item.text}`).join("\n");
  }, []);
  ...
  return (
    <section className={styles.app}>
      ...
      <aside className={`${styles.sidebar} ${sidebarOpen ? styles.sidebarOpen : ""}`}>
        <header className={styles.sidebarHeader}><div><span><Sparkles /></span><strong>ROSTA Insight</strong></div><button type="button" onClick={() => setSidebarOpen(false)} aria-label="Menüyü kapat"><PanelLeftClose /></button></header>
        ...
        <footer className={styles.sidebarFooter}><span className={providerReady ? styles.online : ""} /><div><strong>{providerReady ? "ROSTA Insight hazır" : "Bağlantı yok"}</strong><small>{modelLabel} · admin + web + vision</small></div></footer>
      </aside>

      <main className={styles.main}>
        <header className={styles.topbar}>
          <button className={styles.menuButton} type="button" onClick={() => setSidebarOpen(true)} aria-label="Geçmişi aç"><Menu /></button>
          <button className={styles.model} type="button"><span>ROSTA Insight</span><small>{modelLabel}</small><ChevronDown /></button>
