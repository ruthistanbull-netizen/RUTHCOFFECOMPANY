...
type DockState = "occupied" | "empty" | "near";
...
const DOCK_SELECTOR = "[data-ruthie-orb-dock]";
...
function magneticRadius() {
  return isMobileViewport() ? 170 : 205;
}

function snapRadius() {
  return isMobileViewport() ? 112 : 138;
}
...
function setDockVisualState(state: DockState) {
  const dock = dockElement();
  if (dock) dock.dataset.state = state;
}

function dockPosition(): OrbPosition | null {
  const dock = dockElement();
  if (!dock) return null;
...
function centerDistance(position: OrbPosition, target: OrbPosition) {
  return Math.hypot(position.x - target.x, position.y - target.y);
}

function defaultPosition(): OrbPosition {
  return dockPosition() || clampPosition({
    x: window.innerWidth - currentOrbSize() - 22,
...
export function RuthieDockedQuickVoiceOrb() {
  ...
  const cancelMotionFrame = useCallback(() => {
  ...
  }, [applyPosition, cancelMotionFrame, resetMotionVisual]);

  const snapToDock = useCallback((initialVelocityX = 0, initialVelocityY = 0) => {
    const initial = positionRef.current;
    const initialTarget = dockPosition();
    if (!initial || !initialTarget) return false;

    cancelMotionFrame();
    dockedRef.current = false;
    setDockVisualState("near");
    setGliding(true);

    let current = { ...initial };
    let velocityX = initialVelocityX * 1000;
    let velocityY = initialVelocityY * 1000;
    ...
    const tick = (time: number) => {
      ...
      previousTime = time;
      elapsed += dt;
      const target = dockPosition() || initialTarget;
      const dx = target.x - current.x;
      const dy = target.y - current.y;
      const accelerationX = (stiffness * dx - damping * velocityX) / mass;
      const accelerationY = (stiffness * dy - damping * velocityY) / mass;

      velocityX += accelerationX * dt;
      velocityY += accelerationY * dt;
      current = {
        x: current.x + velocityX * dt,
        y: current.y + velocityY * dt,
      };

      applyPosition(current);
      updateMotionVisual(velocityX / 1000, velocityY / 1000);

      const distance = centerDistance(current, target);
      const speed = Math.hypot(velocityX, velocityY);
      if ((distance < 0.45 && speed < 8) || elapsed > 1.15) {
        applyPosition(target);
        savePosition(target);
        inertiaFrameRef.current = null;
        dockedRef.current = true;
        setDockVisualState("occupied");
        setGliding(false);
        window.requestAnimationFrame(resetMotionVisual);
        return;
      }
    ...
    inertiaFrameRef.current = window.requestAnimationFrame(tick);
    return true;
  }, [applyPosition, cancelMotionFrame, resetMotionVisual, updateMotionVisual]);
  ...
  const startInertia = useCallback((initialVelocityX: number, initialVelocityY: number) => {
    ...
    if (target && centerDistance(initial, target) <= snapRadius()) {
      snapToDock(initialVelocityX, initialVelocityY);
      return;
    }

    cancelMotionFrame();
    dockedRef.current = false;
    setDockVisualState("empty");
    ...
    const tick = (time: number) => {
      ...
      const friction = Math.pow(outsideX || outsideY ? 0.865 : 0.946, dt / 16.667);
      velocityX *= friction;
      velocityY *= friction;
      current = { x: nextX, y: nextY };

      const magneticTarget = dockPosition();
      if (magneticTarget && centerDistance(current, magneticTarget) <= snapRadius()) {
        positionRef.current = current;
        inertiaFrameRef.current = null;
        snapToDock(velocityX, velocityY);
        return;
      }
    ...
    inertiaFrameRef.current = window.requestAnimationFrame(tick);
  }, [applyPosition, cancelMotionFrame, resetMotionVisual, snapToDock, updateMotionVisual]);

  const magnetizePosition = useCallback((raw: OrbPosition) => {
    const target = dockPosition();
    if (!target) return raw;
    const distance = centerDistance(raw, target);
    ...
    const attraction = Math.min(0.58, 0.08 + closeness * closeness * 0.52);
    return {
      x: raw.x + (target.x - raw.x) * attraction,
      y: raw.y + (target.y - raw.y) * attraction,
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      if (pressedRef.current || draggingRef.current || now < suppressResizeUntilRef.current || !widthChanged) return;
      ...
      resizeTimerRef.current = window.setTimeout(() => {
        ...
        const next = positionRef.current ? clampPosition(positionRef.current) : (readStoredPosition() || defaultPosition());
        applyPosition(next);
        savePosition(next);
        resetMotionVisual();
      }, 120);
  ...
  useEffect(() => {
    const shouldListen = (pressed || listeningLatched) && !dragging;
    if (shouldListen && voiceReady && !realtime.connected && realtime.phase !== "connecting") ensureConnection();
  ...
  const onPointerDown = (event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    gestureRef.current = {
      ...
      pointerType,
      interaction: beginInteraction({ pointerType, x: event.clientX, y: event.clientY, at: now, startedOnHandle: true }),
      ...
      velocityY: 0,
      listeningAtStart,
      releaseBackgroundLock: pointerType === "touch" || pointerType === "pen" ? acquireBackgroundInteractionLock() : null,
    };
  ...
  const finishPointerGesture = (event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    if (wasDragging) {
      ...
      const target = dockPosition();
      if (!cancelled && current && target && centerDistance(current, target) <= snapRadius()) {
        snapToDock(gesture.velocityX, gesture.velocityY);
      } else if (cancelled) {
        stopInertia(true);
        setDockVisualState("empty");
      } else {
  ...
  return createPortal(
    <>
      ...
      <div ref={positionLayerRef} className={styles.positionLayer} data-ready={positionReady ? "true" : "false"}>
        <button
          ref={buttonRef}
          type="button"
