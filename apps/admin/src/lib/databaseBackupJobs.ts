...
type BackupGlobal = typeof globalThis & {
  __rostaDatabaseBackupJobs?: Map<string, DatabaseBackupJob>;
};

const backupGlobal = globalThis as BackupGlobal;
const jobs = backupGlobal.__rostaDatabaseBackupJobs || new Map<string, DatabaseBackupJob>();
backupGlobal.__rostaDatabaseBackupJobs = jobs;
...
async function runBackup(job: DatabaseBackupJob, supabase: SupabaseAdminLike) {
  try {
    ...
    const { data: manifestData, error: manifestError } = await supabase.rpc("rosta_backup_manifest");
    ...
    entries.push({
      name: "README.txt",
      data: [
        "ROSTA Coffee Co. self-hosted Supabase veri yedeği",
    ...
    for (let tableIndex = 0; tableIndex < tables.length; tableIndex += 1) {
      ...
      while (true) {
        const { data, error } = await supabase.rpc("rosta_backup_table_chunk", {
          p_schema: schemaName,
          p_table: tableName,
          p_offset: offset,
          p_limit: CHUNK_SIZE,
    ...
    entries.push({
      ...
      data: JSON.stringify({
        ...
        format: "rosta-supabase-jsonl-v1",
    ...
    const filename = `rosta-supabase-backup-${stamp()}.zip`;
