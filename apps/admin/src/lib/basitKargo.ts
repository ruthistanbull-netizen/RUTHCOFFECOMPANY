...
export async function basitKargoRequest<T = unknown>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!headers.has("X-Request-Id")) headers.set("X-Request-Id", `rosta-${crypto.randomUUID()}`);
  headers.set("X-ROSTA-Retry-Owner", "basit-kargo-client");
  ...
  try {
    response = await fetch(`${apiBaseUrl()}/label/svg/${encodeURIComponent(id)}`, {
      headers: {
        ...
        "X-Request-Id": `rosta-label-${crypto.randomUUID()}`,
      },
