...
export function lifecycleCopy(lifecycle: OrderLifecycle, order: any) {
  ...
  if (lifecycle === "received") return { subject: `Siparişini aldık - ${orderNo}`, eyebrow: "Sipariş Alındı", headline: "Siparişin başarıyla alındı", intro: `${orderNo} numaralı siparişin sistemimize ulaştı. Hazırlama süreci başladığında seni yeniden bilgilendireceğiz.`, accent: "#B9563D" };
  if (lifecycle === "preparing") return { subject: `Siparişin hazırlanıyor - ${orderNo}`, eyebrow: "Hazırlanıyor", headline: "Siparişini hazırlıyoruz", intro: `${orderNo} numaralı siparişin hazırlama sürecine alındı. Ürünlerin kontrol edilip paketlendikten sonra kargoya hazır olacak.`, accent: "#B9563D" };
  if (lifecycle === "ready") return { subject: `Siparişin kargoya hazır - ${orderNo}`, eyebrow: "Kargoya Hazır", headline: "Siparişin gönderime hazırlandı", intro: `${orderNo} numaralı siparişin için kargo kodu oluşturuldu. Kargon firmaya teslim edildiğinde yeniden haber vereceğiz.`, accent: "#B9563D" };
  if (lifecycle === "shipped") return { subject: `Siparişin yola çıktı - ${orderNo}`, eyebrow: "Gönderildi", headline: "Siparişin kargoya verildi", intro: `${orderNo} numaralı siparişin yola çıktı. Güncel hareketleri sipariş takip sayfasından kontrol edebilirsin.`, accent: "#7D3E31" };
  ...
  return { subject: `Siparişin teslim edildi - ${orderNo}`, eyebrow: "Teslim Edildi", headline: "Siparişin sana ulaştı", intro: `${orderNo} numaralı siparişin teslim edildi. ROSTA Coffee Co.’yu tercih ettiğin için teşekkür ederiz.`, accent: "#55704B" };
}

export function buildOrderLifecycleHtml(lifecycle: OrderLifecycle, order: any) {
  const copy = lifecycleCopy(lifecycle, order);
  const siteUrl = clean(process.env.NEXT_PUBLIC_SITE_URL || process.env.PUBLIC_SITE_URL) || "https://rostacoffecompany.zeabur.app";
