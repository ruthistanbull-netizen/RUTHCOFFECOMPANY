      ...
      <div className="mx-auto max-w-5xl">
          ...
          {cards.map((item, index) => {
            ...
            const content = (
              <div className="h-full rounded-lg border border-kraft/35 bg-carbon-soft p-7 transition  focus-visible:border-brick  ">
                ...
                <p className="mt-4 leading-7 text-[var(--rosta-action-text)]/70">{item.desc}</p>
                {item.actionHref && item.actionLabel ? (
                  <a
                    href={item.actionHref}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-6 inline-flex w-full items-center justify-center rounded-full bg-brick px-5 py-3 text-[10px] uppercase tracking-wide-luxe text-[var(--rosta-action-text)] transition active:bg-espresso"
                  >
                    {item.actionLabel}
                  </a>
                ) : null}
        ...
        <AnimatedBlock delay={0.26}>
          <section className="mt-12 rounded-[2rem] border border-kraft/35 bg-carbon-soft p-6 md:p-10">
            ...
            <div className="grid gap-5 text-sm leading-7 text-[var(--rosta-action-text)]/70 md:grid-cols-2">
              <div className="space-y-2">
                <p><strong className="text-[var(--rosta-action-text)]">Resmî satıcı:</strong> Görkem Çirik</p>
                <p><strong className="text-[var(--rosta-action-text)]">İşletme türü:</strong> Şahıs işletmesi</p>
                <p><strong className="text-[var(--rosta-action-text)]">Vergi dairesi / no:</strong> Bayrampaşa / 2571309841</p>
              </div>
              <div className="space-y-2">
                {supportEmail ? <p><strong className="text-[var(--rosta-action-text)]">E-posta:</strong> <a className="hover:text-[var(--rosta-action-text)]" href={`mailto:${supportEmail}`}>{supportEmail}</a></p> : null}
                <p><strong className="text-[var(--rosta-action-text)]">Adres:</strong> Vatan Mahallesi, Küçük Sokak No: 2 Daire: 1, Bayrampaşa / İstanbul</p>
              </div>
            </div>
          </section>
        </AnimatedBlock>
        ...
        <AnimatedBlock delay={0.3}>
          <section id="contact-form" className="mt-16 scroll-mt-32 rounded-[2rem] border border-kraft/35 bg-carbon-soft p-6 md:p-10">
            <div className="mb-9 max-w-2xl">
              ...
              <p className="mt-5 leading-7 text-[var(--rosta-action-text)]/70">
        ...
        <AnimatedBlock delay={0.34}>
          <Link href="/products" className="mt-10 inline-flex rounded-full bg-brick px-8 py-4 text-xs uppercase tracking-wide-luxe text-[var(--rosta-action-text)] transition active:bg-espresso">
            Alışverişe Devam Et
          </Link>
        </AnimatedBlock>
      </div>
