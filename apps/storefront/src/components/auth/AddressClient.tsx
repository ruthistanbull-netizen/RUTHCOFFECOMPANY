  ...
  if (isLoading || isFetching) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-carbon px-4 pt-20" role="status" aria-busy="true">
        <LoadingIndicator size="lg" className="text-brick" label="Adresler yükleniyor" />
      </div>
    );
  }
        ...
        <section className="rounded-2xl border border-kraft/35 bg-carbon-soft p-5 md:p-6">
          <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 bg-brick px-6 py-3 text-xs uppercase tracking-wide-luxe text-[var(--rosta-action-text)] transition active:bg-espresso"
            >
          ...
          {formOpen && (
            <form onSubmit={addAddress} className="mb-6 rounded-xl border border-kraft/25 bg-carbon p-4 md:p-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70">Ad soyad<input required value={form.fullName} onChange={(event) => updateField("fullName", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70">Telefon<input required value={form.phone} onChange={(event) => updateField("phone", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70">E-posta<input type="email" value={form.email} onChange={(event) => updateField("email", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70">Il<input required value={form.city} onChange={(event) => updateField("city", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70">Ilçe<input required value={form.district} onChange={(event) => updateField("district", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70">Mahalle<input value={form.neighborhood} onChange={(event) => updateField("neighborhood", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70 md:col-span-2">Açık adres<textarea required rows={3} value={form.addressLine} onChange={(event) => updateField("addressLine", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
                <label className="text-xs uppercase tracking-wide-luxe text-cream/70">Posta kodu<input value={form.postalCode} onChange={(event) => updateField("postalCode", event.target.value)} className="mt-2 w-full rounded-xl border border-kraft/35 bg-carbon-soft px-4 py-3 text-sm text-cream outline-none transition focus:border-brick" /></label>
              </div>
              ...
              <button
                ...
                className="mt-5 inline-flex items-center justify-center gap-2 bg-brick px-7 py-4 text-xs uppercase tracking-wide-luxe text-[var(--rosta-action-text)] transition active:bg-espresso disabled:opacity-60"
              >
          ...
          {addresses.length === 0 ? (
            <div className="rounded-xl border border-kraft/25 bg-carbon p-8 text-center">
              <MapPin className="mx-auto mb-4 text-brick" size={30} />
              <p className="font-heading text-xl">Henüz kayıtlı adres yok</p>
              <p className="mt-3 text-sm text-cream/70">Adres ekleyebilirsin. Sipariş verirsen kullandığın adres de otomatik buraya eklenecek.</p>
              <button type="button" onClick={() => setFormOpen(true)} className="mt-6 inline-block bg-brick px-7 py-4 text-xs uppercase tracking-wide-luxe text-[var(--rosta-action-text)]">Adres Ekle</button>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {addresses.map((address) => {
                ...
                return (
                  <div key={address.id} className="rounded-xl border border-kraft/25 bg-carbon p-5">
                    ...
                    <p className="text-sm leading-7 text-cream/70">{address.address_line}<br />{address.neighborhood ? `${address.neighborhood} / ` : ""}{address.district} / {address.city}{address.postal_code ? ` · ${address.postal_code}` : ""}</p>
