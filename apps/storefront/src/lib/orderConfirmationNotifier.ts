...
const CANONICAL_ADMIN_INTERNAL_URL = "https://ruthcommerce.zeabur.app";
...
function adminInternalUrls() {
  const configured = process.env.ADMIN_INTERNAL_URL?.trim().replace(/\/$/, "") || "";
    ...
    try {
      const response = await fetch(`${baseUrl}/api/internal/order-confirmation`, {
        ...
        headers: {
          ...
          "x-ruth-timestamp": timestamp,
          "x-ruth-signature": signature,
        },
        body,
        cache: "no-store",
...
export async function notifyOrderConfirmationEmail(orderId: string) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Sipariş onay maili gönderilemedi.");
}
