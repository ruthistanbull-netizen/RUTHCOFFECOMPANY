...
expect(!sharedDocker.includes("npm run build:all"), "root Dockerfile must never couple panel and storefront builds");
expect(!sharedDocker.includes("ROSTA_APP"), "root Dockerfile must not use dynamic app selection");
expect(!sharedDocker.includes("ZEABUR_SERVICE_NAME"), "root Dockerfile must not guess the Zeabur service at runtime");
expect(!sharedDocker.includes(".rosta-app-target"), "root Dockerfile must not persist a dynamic runtime target");
expect(sharedDocker.includes("RUN npm run build:admin"), "root Dockerfile must build the admin app only");
expect(sharedDocker.includes('CMD ["npm","run","start:admin"]'), "root Dockerfile must start the admin app only");
expect(sharedDocker.includes("ENV PORT=3000") && sharedDocker.includes("EXPOSE 3000"), "root Dockerfile must match the Ruth-proven port 3000 contract");
expect(panelDocker.includes("ENV PORT=3000") && panelDocker.includes("EXPOSE 3000"), "panel Dockerfile must match the Ruth-proven port 3000 contract");
expect(storefrontDocker.includes("ENV PORT=3000") && storefrontDocker.includes("EXPOSE 3000"), "storefront Dockerfile must match the Ruth-proven port 3000 contract");
...
expect(exactRuntimeServiceDocker.includes("apps/storefront/.next/BUILD_ID"), "storefront Docker alias must verify the storefront build artifact");
